...
def plot_RTT(infile,burnin, file_stem="",one_file=False, root_plot=0, plot_type=1):
	...
	if platform.system() == "Windows" or platform.system() == "Microsoft":
	 Rfile+= "\n\nfile_path=paste0(getwd(), '/', quote(%s))\n" % (name_file) # 9
	 Rfile+= "\n\npdf(file=paste0(file_path, '_RTT.pdf'),width=0.6*9,height=%s)\npar(mfrow=c(%s,1))" % (0.6*7*n_plots,n_plots) # 9
	else:
	...
	def RTT_plot_in_R(args, alpha):
		...
		if plot_type==1:
			R_code += "\nplot(age,age,type = 'n', ylim = c(0, max(1/M_mean, na.rm = TRUE)), xlim = c(%s,%s), ylab = 'Longevity (Myr)', xlab = 'Ma' )" % (max_x_axis,min_x_axis)
